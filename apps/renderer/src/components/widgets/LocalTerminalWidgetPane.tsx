/**
 * LocalTerminalWidgetPane should remain session-type agnostic.
...
type Props = {
  tab: LocalTerminalWidgetTabRecord;
  isActive: boolean;
  ...
  onStatus: (tabId: string, status: WidgetLifecycleStatus) => void;
  onWsConnected: (tabId: string, connected: boolean) => void;
};

export function LocalTerminalWidgetPane({
  tab,
  isActive,
  onTraffic,
  onCommandChannel,
  ...
  useEffect(() => {
    console.log(`[LocalTerminalWidgetPane] WS connect effect triggered - tabId=${tab.id.slice(0, 8)}, port=${port}, terminalReady=${terminalReady}, status=${tab.status}, hasTermRef=${!!xtermRef.current}, hasFitRef=${!!fitRef.current}`);

    if (!port) {
      console.log(`[LocalTerminalWidgetPane] Skipping WS connect - no port yet (tabId=${tab.id.slice(0, 8)})`);
      return;
    }
    if (!terminalReady) {
      console.log(`[LocalTerminalWidgetPane] Skipping WS connect - terminal not ready yet (tabId=${tab.id.slice(0, 8)})`);
      return;
    }
    const term = xtermRef.current;
    const fit = fitRef.current;
    if (!term || !fit) {
      console.log(`[LocalTerminalWidgetPane] Skipping WS connect - term or fit not available (tabId=${tab.id.slice(0, 8)}), will retry in 100ms`);
      ...
      const retryTimer = setTimeout(() => {
        console.log(`[LocalTerminalWidgetPane] Retrying WS connect for tabId=${tab.id.slice(0, 8)}`);
    ...
    console.log(`[LocalTerminalWidgetPane] Connecting WebSocket to port ${port} (tabId=${tab.id.slice(0, 8)}, generation=${generation})`);
    ...
    return () => {
      console.log(`[LocalTerminalWidgetPane] Cleaning up WS connection for tabId=${tab.id.slice(0, 8)}, port=${port}, generation=${generation}`);
